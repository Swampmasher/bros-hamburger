...
Citizen.CreateThread(function()
	while true do
		...
		if (distance < 16) then
			...
			if distance < 3 then
				isInMarker = true
				DrawText3D(-1178.15, -891.037, 13.768 + 0.4, '~g~E~s~ - Meslek Aracı Çıkart')
			end
		end
		...
		if (distance2 < 16) then
			...
			if distance2 < 3 then
				isInMarker2 = true
				DrawText3D(-1164.05, -891.238, 14.117 + 0.4, '~g~E~s~ - Meslek Aracını Geri Ver')
			end
		end
...
Citizen.CreateThread(function()
    while true do
        ...
        if distance < 10 then
            ...
            if distance < 1 then
                DrawText3D(Config.BurgerEtPisirme.x, Config.BurgerEtPisirme.y, Config.BurgerEtPisirme.z + 0.4, '~g~[E]~s~ - Hamburger Eti Pisir')
        ...
        if distance3 < 10 then
            ...
            if distance3 < 1 then
                DrawText3D(Config.BurgerPaketleme.x, Config.BurgerPaketleme.y, Config.BurgerPaketleme.z + 0.4, '~g~[E]~s~ - Hamburger Paketle')
                if IsControlJustReleased(0, 38) then
                    BurgerPaketle()
                end
            end
        ...
        if distance2 < 1 then
            DrawText3D(Config.BurgerHazirlama.x, Config.BurgerHazirlama.y, Config.BurgerHazirlama.z + 0.4, '~g~[E]~s~ - Hamburger Hazırla')
            if IsControlJustReleased(0, 38) then
                burgermenu()
            end
        end
        ...
        if distance7 < 10 then
            ...
            if distance7 < 1 then
                DrawText3D(Config.PatatesPisir.x, Config.PatatesPisir.y, Config.PatatesPisir.z + 0.4, '~g~[E]~s~ - Patates Pişir')
                if IsControlJustReleased(0, 38) then
                    Patates()
                end
            end
...
function burgermenu()
	...
	local elements ={  
    { label = 'Normal Burger', value = 'burger'},
    { label = 'Tavuk Burger', value = 'dburger'},
	}
	
	ESX.UI.Menu.CloseAll() 
	ESX.UI.Menu.Open('default', GetCurrentResourceName(),'ventacyc',{
		title = 'Hangi burger yapmak istiyosun?', 
		align = 'top-right',
		elements = elements  
    ...
    elseif data.current.value == 'dburger' then
        ...
        exports['mythic_progbar']:Progress({
            name = "dburger",
            duration = 3250,
            label = 'Tavuk Burger Hazırlıyorsun.',
            useWhileDead = false,
            canCancel = true,
            controlDisables = {
                disableMovement = true,
...
function pisirmenu()
	local elements ={  
        { label = 'Köfte', value = 'burger'},
        { label = 'Tavuk Köfte', value = 'tavukofte'},
        ...
        elseif data.current.value == 'tavukofte' then
            ...
            exports['mythic_progbar']:Progress({
                name = "balikli_burger",
                duration = 3250,
                label = 'Tavuk Köfte Pişiriyorsun',
                useWhileDead = false,
                canCancel = true,
                controlDisables = {
                    disableMovement = true,
...
function malzememenu()
    ...
    { label = 'Susamlı Ekmek', value = 'susamlıekmek'},
	}
...
function BurgerPaketle()
	...
	local elements ={  
    { label = 'Burger Paketle', value = 'burger'},
    { label = 'Tavuk Burger Paketle', value = 'tavukburger'},
	}
