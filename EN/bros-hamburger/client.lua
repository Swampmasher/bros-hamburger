...
Citizen.CreateThread(function()
	while true do
		...
		if (distance < 16) then
			...
			if distance < 3 then
				isInMarker = true
				DrawText3D(-1178.15, -891.037, 13.768 + 0.4, '~g~E~s~ - Get Car')
			end
		end
		...
		if (distance2 < 16) then
			...
			if distance2 < 3 then
				isInMarker2 = true
				DrawText3D(-1164.05, -891.238, 14.117 + 0.4, '~g~E~s~ - Delete Car')
			end
		end
	...
	if not aractami then
		if not aracalindi then
			ESX.Game.SpawnVehicle('rumpo2', vector3(-1165.46, -887.827, 13.572), 121.07, function(vehicle)
				local plate = 'BURGERSHOT' .. math.random(100, 900)
			...
			ESX.ShowNotification('You already have a vehicle!')
		end
	else
		ESX.ShowNotification('You cannot do this while in a vehicle!')
	end
end
			...
			if player == driver then
				if model == -1776615689 then
					...
					ESX.TriggerServerCallback("bros-jobveh:givemoney", function(CB)
						if CB then
						  ESX.ShowNotification('$2500 received as a deposit has been returned to your bank account!')
						end
					end, 2500)
				else
					ESX.ShowNotification('This tool is not a professional tool!')
				end
			else
				ESX.ShowNotification('You are not the driver of the vehicle!')
			end
    	else
        	ESX.ShowNotification('You havent bought a profession tool!')
		end
	else
...
Citizen.CreateThread(function()
    while true do
        ...
        if distance < 10 then
            ...
            if distance < 1 then
                DrawText3D(Config.BurgerEtPisirme.x, Config.BurgerEtPisirme.y, Config.BurgerEtPisirme.z + 0.4, '~g~[E]~s~ - Grilled Hamburger Meat')
        ...
        if distance4 < 10 then
            ...
            if distance4 < 1 then
                DrawText3D(Config.BurgerSatisyeri.x, Config.BurgerSatisyeri.y, Config.BurgerSatisyeri.z + 0.4, '~g~[E]~s~ - Sell Hamburgers')
                if IsControlJustReleased(0, 38) then
                    BurgerSat()
                end
            end
        ...
        if distance6 < 10 then
            ...
            if distance6 < 1 then
                DrawText3D(Config.Malzeme.x, Config.Malzeme.y, Config.Malzeme.z + 0.4, '~g~[E]~s~ - Material Cabinet')
                if IsControlJustReleased(0, 38) then
                  malzememenu()
                end
            end
        ...
        if distance2 < 1 then
            DrawText3D(Config.BurgerHazirlama.x, Config.BurgerHazirlama.y, Config.BurgerHazirlama.z + 0.4, '~g~[E]~s~ - Prepare a Hamburger')
            if IsControlJustReleased(0, 38) then
                burgermenu()
            end
        end
        ...
        if satiyormu == true and satiskordinat ~= false and distance5 < 10 then
            ...
            if distance5 < 1 then
                DrawText3D(coords[1], coords[2], coords[3] + 0.4, '~g~[E]~s~ - Knock on the door')
                if IsControlJustReleased(0, 38) then
                    BurgeryuSat()
                end
            end
        ...
        if distance7 < 10 then
            ...
            if distance7 < 1 then
                DrawText3D(Config.PatatesPisir.x, Config.PatatesPisir.y, Config.PatatesPisir.z + 0.4, '~g~[E]~s~ - Bake Potatoes')
                if IsControlJustReleased(0, 38) then
                    Patates()
                end
            end
    ...
    if not malzemealiyormu then
        ...
        exports['mythic_progbar']:Progress({
            ...
            if not cancelled then
                ...
                ESX.ShowNotification('You Cooked Potatoes!')
             else
...
AddEventHandler('bros-burger:koftepisir', function()
    if not pisiriyormu then
        ...
        if finished ~= 100 then
            ...
            ESX.ShowNotification('you couldnt cook the meat!')
        else
            ...
            if finished2 ~= 100 then
                ...
                if finished3 ~= 100 then
                    ...
                    pisiriyormu = false
                    ClearPedTasksImmediately(player)
                    ESX.ShowNotification('You have successfully cooked the meat!')
                end
            end
        end
    end
...
function BurgerSat()
    if not satiyormu then
        ESX.TriggerServerCallback('bros-burger:malzemekontrol', function(data)
            ...
            satiyormu = true
            ESX.ShowNotification('You started selling burgers!')
        end, "burgerpaketi", 1)
    elseif satiyormu then
        ESX.ShowNotification('You are already selling a burger!')
    end
end
...
function BurgeryuSat()
    exports['mythic_progbar']:Progress({
        ...
        if not cancelled then
            ...
            satiskordinat = false
            satiyormu = false
            ESX.ShowNotification('You have successfully sold a burger!')
        else
            -- Do Something If Action Was Cancelled
        end
    end)
...
function burgermenu()
    ...
    { label = 'Chicken Burger', value = 'dburger'},
	}
	...
	if data.current.value == 'burger' then 
        ...
        exports['mythic_progbar']:Progress({
            name = "burger",
            duration = 3250,
            label = 'You are preparing a hamburger.',
            useWhileDead = false,
            canCancel = true,
            controlDisables = {
                disableMovement = true,
    ...
    elseif data.current.value == 'dburger' then
        ...
        exports['mythic_progbar']:Progress({
            name = "dburger",
            duration = 3250,
            label = 'You Are Preparing Chicken Burger.',
            useWhileDead = false,
            canCancel = true,
            controlDisables = {
                disableMovement = true,
	...
	menu.close()
	TriggerEvent('notification', 'you refused to do!', 2, 2000)
...
function pisirmenu()
	local elements ={  
        { label = 'Meatball', value = 'burger'},
        { label = 'Chicken Meatballs', value = 'tavukofte'},
        ...
        elseif data.current.value == 'tavukofte' then
            ...
            exports['mythic_progbar']:Progress({
                name = "balikli_burger",
                duration = 3250,
                label = 'You are Cooking Chicken Meatballs',
                useWhileDead = false,
                canCancel = true,
                controlDisables = {
                    disableMovement = true,
...
function malzememenu()
	...
	local elements ={  
    { label = 'Bread', value = 'ekmek'},
    { label = 'Ketchup', value = 'ketcap'},
    { label = 'Mayonnaise', value = 'mayonez'},
    { label = 'Lettuce', value = 'marul'},
    { label = 'Chedar', value = 'chedar'},
    { label = 'Sesame Bread', value = 'susamlıekmek'},
	}
    ...
    elseif data.current.value == 'mayonez' then
        ...
        exports['mythic_progbar']:Progress({
            name = "mayonez",
            duration = 2000,
            label = 'You Are Buying Mayonnaise',
            useWhileDead = false,
            canCancel = true,
            controlDisables = {
                disableMovement = true,
    ...
    elseif data.current.value == 'susamlıekmek' then
        ...
        exports['mythic_progbar']:Progress({
            name = "susamlıekmek",
            duration = 2000,
            label = 'You are Buying Sesame Bread',
            useWhileDead = false,
            canCancel = true,
            controlDisables = {
                disableMovement = true,
	...
	menu.close()
	TriggerEvent('notification', 'You refused to do it!', 2, 2000)
	ClearPedTasks(player)
	FreezeEntityPosition(player,false)
end, function(data, menu)
end)
...
function BurgerPaketle()
	...
	local elements ={  
    { label = 'Burger Pack', value = 'burger'},
    { label = 'Chicken Burger Pack', value = 'tavukburger'},
	}
